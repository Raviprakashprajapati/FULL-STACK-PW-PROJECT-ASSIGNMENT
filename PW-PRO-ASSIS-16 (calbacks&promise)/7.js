...
async function combineApi(){

    try {

        // let todoFirst = await fetch('https://jsonplaceholder.typicode.com/todos/1')
        let todoFirst = await fetch('https://jsonplaceholder.typicode.com/todos/1')
        if (!todoFirst.ok) {
            throw todoFirst.statusText
        }
        let Todo = await todoFirst.json()
        
        let postFirst = await fetch('https://jsonplaceholder.typicode.com/posts/1')
        if (!postFirst.ok) {
            throw postFirst.statusText
        }
        let Post = await postFirst.json()
